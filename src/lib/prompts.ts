...
export const PROMPT_VERSION = "2026-07-11-quality-2";

const BASE_OUTPUT_RULES = `通用输出规则：
- 使用中文回答，除非用户明确要求其他语言。
- 先给结论，再给要点；避免开场白、客套话和重复用户问题。
- 默认结构：
  【结论】一句话说明核心意思。
  【要点】2-4 条短要点，优先解释用户最可能关心的点。
  【补充】只写必要背景、例子、风险或可能的 OCR 误差。
- 不确定时直接说明不确定点，不编造事实。
- 控制长度在 120-260 个中文字；复杂内容可以略长。`;
...
- 【结论】直接给自然中文译文。
- 【要点】解释关键词、句子结构或语气。
- 【补充】如果 OCR 结果疑似有错，指出可能错字；如果没有明显问题，不要强行提 OCR。`,
